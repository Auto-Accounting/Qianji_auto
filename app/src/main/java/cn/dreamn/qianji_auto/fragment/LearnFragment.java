...
import cn.dreamn.qianji_auto.core.SimpleRecyclerAdapterLearn;
...
public class LearnFragment extends BaseFragment {
    ...
    private SimpleRecyclerAdapterLearn mAdapter;
    ...
    protected void initViews() {
        ...
        recyclerView.setAdapter(mAdapter = new SimpleRecyclerAdapterLearn());
    ...
    private OnItemMenuClickListener mMenuItemClickListener = (menuBridge, position) -> {
        ...
        if (direction == SwipeRecyclerView.RIGHT_DIRECTION) {
            ...
            if(menuPosition==0){
                Storage.type(Storage.Learn).del(key);
                refresh();
            }else{
                showAddDialog("修改语义规则",key,value,(from,to)->{
                    Storage.type(Storage.Learn).set(from,to);
    ...
    private void loadData() {
        new Handler().postDelayed(() -> {
            ...
            if(data!=null&&data.size()!=0){
                map_error.setLeftString(String.format("已有%d条语义规则", data.size()));
    ...
    protected void showAddDialog(String title,String from,String to,CallBack callBack) {
        ...
        final View textEntryView = factory.inflate(R.layout.input_learn, null);
